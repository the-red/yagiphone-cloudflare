...
describe('playback/dial routes', () => {
  ...
  it('/replay: 録音なしは「録音がありません」', async () => {
    ...
    const res = await app.request(`/twilio/replay?To=${encodeURIComponent(TO)}`, {}, env);
  ...
  it('/play: テナント名と録音者名・Playを返す', async () => {
    await seedContact({ tenant_id: 'hosoiri', phone_number: '+8190', contact_type: 'recorder', name: '太郎' });
    const url = `/twilio/play?TenantID=hosoiri&Recorder=${encodeURIComponent('+8190')}&RecordingUrl=${encodeURIComponent('https://x/a.mp3')}`;
  ...
  it('/dial: 各listenerをQueueにenqueueし空Responseを返す', async () => {
    ...
    const url = `/twilio/dial?TenantID=hosoiri&Caller=${encodeURIComponent('+8190')}&RecordingUrl=${encodeURIComponent('https://x/a.mp3')}`;
    ...
    expect(sent.every((m) =>
      m.body.playUrl.includes('https://h.example/twilio/play?Recorder=')
  ...
  it('/dial: TWILIO_VALIDATE=true かつ正当な署名で 200、enqueue される', async () => {
    ...
    const sent: any[] = [];
    const mockQueue = { sendBatch: async (msgs: any[]) => { sent.push(...msgs); }, send: async () => {} };
    const path = `/twilio/dial?TenantID=hosoiri&Caller=${encodeURIComponent('+8190')}&RecordingUrl=${encodeURIComponent('https://x/a.mp3')}`;
